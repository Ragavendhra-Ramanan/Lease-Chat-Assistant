    ...
    async def run(self, state:AgentState):
        ...
        if context == "vehicle":
            ...
            if step == "preowned":
                ...
                state.quote_intermediate_results = f"Do you want a Preowned vehicle? Options: {options}"

            elif step == "make":
                options = df["Make"].unique().tolist()
                state.quote_intermediate_results = f"Please choose a Make: {options}"
                ...
                state.quote_intermediate_results = f"Please choose a Country: {options}"

            elif step == "model":
                options = df["Model"].unique().tolist()
                state.quote_intermediate_results = f"Please choose a Model: {options}"

            elif step == "year":
                options = df["Year"].unique().tolist()
                state.quote_intermediate_results = f"Please choose a Year: {options}"
        ...
        # --- PRODUCT filtering ---
            ...
            if step == "product_name":
                ...
                state.quote_intermediate_results = f"Please choose a Product: {options}"

            elif step == "lease_term":
                options = df["Lease Term"].unique().tolist()
                state.quote_intermediate_results = f"Select Lease Term: {options}"

            elif step == "flexi_lease":
                options = df["Flexi Lease"].unique().tolist()
                state.quote_intermediate_results = f"Flexi Lease available? Options: {options}"

            elif step == "tax_saving_plan":
                options = df["Tax Saving Plan"].unique().tolist()
                state.quote_intermediate_results = f"Tax Saving Plan required? Options: {options}"

            elif step == "renewal_cycle":
                options = df["Renewal Cycle"].unique().tolist()
                state.quote_intermediate_results = f"Please choose Renewal Cycle: {options}"

            elif step == "maintenance_type":
                options = df["Maintenance Type"].unique().tolist()
                state.quote_intermediate_results = f"Select Maintenance Type: {options}"
